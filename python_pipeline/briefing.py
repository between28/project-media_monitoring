...
from .utils import format_datetime, format_readable_datetime, limit_text
...
def generate_briefing(connection, config: dict, output_path: str | None = None) -> str:
    ...
    if not candidates:
        ...
        replace_briefing_sections(connection, sections)
        if output_path:
            with open(output_path, "w", encoding="utf-8") as handle:
                handle.write(sections[0]["content"] + "\n")
...
def build_overall_summary(candidates: list[dict], frame_counts: dict, theme_groups: list[dict], config: dict, analysis_now) -> str:
    ...
    if frame_counts.get("비판 / 우려", 0) > 0:
        ...
        lines.append("브리핑 시에는 공급 물량, 후속 일정, 집행 절차를 일관된 메시지로 제시하는 것이 적절합니다.")
    return "\n".join(lines)
...
def build_main_coverage_section(theme_groups: list[dict]) -> str:
    ...
    return "\n".join(
        f"- {group['label']}: {group['sourceSummary']} 등 {group['count']}건. 대표 기사: [{group['lead']['source_name']}] {limit_text(group['lead']['title'], 90)}"
...
def build_issue_section(frame_counts: dict, theme_groups: list[dict]) -> str:
    lines = []
    if frame_counts.get("비판 / 우려", 0) > 0:
        lines.append("- 공급 실현 가능성, 사업 속도, 주민 수용성과 관련한 우려 신호가 반복적으로 나타났습니다.")
    ...
    if frame_counts.get("정책 설명", 0) > 0:
        lines.append("- 후보지, 물량, 일정, 후속 절차 등 정책 세부 설명 수요가 여전히 높습니다.")
    if frame_counts.get("긍정 평가", 0) > 0:
        lines.append("- 일부 보도는 공급 확대 기대와 시장 안정 효과를 긍정적으로 평가했습니다.")
...
def build_impact_section(candidates: list[dict]) -> str:
    return "\n".join(
        f"{index}. [{record['source_name']}] {record['title']} (중요도 {record['importance_score']}, 프레임 {record.get('frame_category') or '기타'})"
...
def build_response_points(frame_counts: dict, theme_groups: list[dict]) -> str:
    ...
    if frame_counts.get("비판 / 우려", 0) > 0:
        lines.append("- 실효성 및 속도 우려에는 단계별 집행계획과 관리지표를 중심으로 대응 포인트를 준비합니다.")
    if frame_counts.get("정치 / 기관 이슈", 0) > 0:
...
def build_theme_groups_for_briefing(records: list[dict], config: dict) -> list[dict]:
    ...
    for record in records:
        ...
        groups[theme_key]["sources"].add(record["source_name"])
...
def get_unique_source_count(records: list[dict]) -> int:
    return len({record["source_name"] for record in records})


def build_supporting_articles(records: list[dict]) -> str:
    return "\n".join(
        f"[{record['source_name']}] {limit_text(record['title'], 70)}"
